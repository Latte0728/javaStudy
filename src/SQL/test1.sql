...
insert into test1 values (default,'가나다',55,'제주','남자');
...
-- '파파파/차차차/사삿'의 성별을 '남자'로 변경
update test1 set gender='남자' where name in ('파파파','차차차', '사사사');

-- 정렬 : order by 필드명 _옵션__	:		옵션 생략시는 오름차순(asc), 내림차순(desc)
select * from test1 order by name; --오름차순
select * from test1 order by name desc; -- 내림차순
...
select * from test1 where (address='서울' address='청주') and age<=20;
